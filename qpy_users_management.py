...
class User():
    ...
    def remove_job(self, jobID):
        ...
        for job in self.cur_jobs:
            if (job.ID == jobID):
                ...
                qpynodes.nodes[job.node].n_used_cores -= job.n_cores
                qpynodes.nodes[job.node].req_mem -= job.mem
                qpynodes.N_used_cores -= job.n_cores
    ...
    def add_job(self, job):
        ...
        qpynodes.N_used_cores += job.n_cores
        if (self.n_used_cores < self.min_cores):
            qpynodes.N_used_min_cores += job.n_cores
    ...
    def request_node(self, jobID, num_cores, mem, node_attr):
        ...
        qpylog.logger.debug('jobID, attribute: '
                            + str(jobID)
                            + str(node_attr) )
        
        qpynodes.N_free_cores = ((qpynodes.N_cores
                                  - qpynodes.N_used_cores)
                                 - (qpynodes.N_min_cores
                                    - qpynodes.N_used_min_cores))
        free_cores = qpynodes.N_free_cores >= num_cores
        ...
        if (space_available):
            best_node = None
            best_free = 0
            qpynodes.nodes_check_lock.acquire()
            if (num_cores == 1):
                for node, info in qpynodes.nodes.iteritems():
                    ...
                    if (info.has_attributes(node_attr) and
                        ...
                        best_free = free
            if (best_node == None):
                for node in qpynodes.nodes_list:
                    if (qpynodes.nodes[node].is_up
                        and info.has_attributes(node_attr)
                        and qpynodes.nodes[node].max_cores
                        - qpynodes.nodes[node].n_outsiders
                        - qpynodes.nodes[node].n_used_cores >= num_cores
                        and qpynodes.nodes[node].total_mem
                        - qpynodes.nodes[node].req_mem > mem):
                        best_node = node
                        break
            qpynodes.nodes_check_lock.release()
            if (best_node == None):
                return 1
...
def distribute_cores():
    ...
    qpynodes.N_min_cores = 0
    qpynodes.N_used_min_cores = 0
    for user in users:
        try:
            users[user].min_cores = users_min[user]
        except:
            users[user].min_cores = 0
        qpynodes.N_min_cores += users[user].min_cores
        qpynodes.N_used_min_cores += min(users[user].min_cores,
    ...
    for user in users:
        users[user].max_cores = (qpynodes.N_cores
                                 - qpynodes.N_min_cores
                                 + users[user].min_cores)
    return 0
